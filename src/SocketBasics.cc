...
/*
  Taken from TimblServer to get rid of the TimblServer dependency.
  Adapted for Wopr.
*/
...
namespace Sockets {
  ...
  bool Socket::write( const string& line ){
	...
	str += this_write;
      ...
      if ( bytes_sent < count ) {
	mess = "write: failed to sent " + to_str(count - bytes_sent) +
	  " bytes out of " + to_str(count);
	return false;
      }
    }
    return true;
  ...
  bool ClientSocket::connect( const string& hostString,
	...
	if ( ::connect( sock, aip->ai_addr, aip->ai_addrlen ) < 0 ){
	  ...
	  mess = string( "ClientSocket: Connection on ") + hostString + ":"
	    + to_str(sock) + " failed (" + strerror(errno) + ")";
	}
      }
