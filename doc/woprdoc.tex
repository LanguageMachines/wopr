...
The last line marked \cmp{L} in the example predicts zero keypresses saved. It
actually finds \cmp{me} after \cmp{m}, but we need to subtract one for the
user's \q{select} action.
...
The esoteric options are \cmp{mode}, and \cmp{keep}. The \cmp{mode:1} argument
specifies that we will send a whole sentence which will be windowed first by
\wopr{}. The default, \cmp{mode:0} indicates we send an \emph{instance} to
\wopr{}. For windowing to work, the context size must be specified as well, with
the parameters \cmp{lc} and \cmp{rc}.
